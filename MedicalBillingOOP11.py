...
class Specialist:
    ...
    def medical_savings(self, amount):
        self.medical_aid_balance =-amount
    ...
    def in_hospital_benefit(self, hospital_cover_balance, ward_fees, theatre_fees, drugs, radiolology_fee):
        ...
        if self.hospital_cover_balance >= self.ward_fees + self.ward_fees + self.theatre_fees + self.drugs + self.radiolology_fee:
            self.hospital_cover_balance = self.hospital_cover_balance - (self.ward_fees + self.theatre_fees + self.drugs + self.radiolology_fee)
...
class Cardiologist(Specialist):
    def __init__(self, medical_aid_balance, pacemaker, stent):
    ...
    def hospital_benefit(self, hospital_cover_balance, ward_fees, theatre_fees, drugs, radiolology_fee, pacemaker, stent):
        new_balance = Specialist.in_hospital_benefit(self, hospital_cover_balance - pacemaker - stent, ward_fees, theatre_fees, drugs, radiolology_fee)
        return new_balance
    

class Dentist(Specialist):
    def __init__(self, medical_aid_balance, teeth_whitinging, cavity_refill):
        Specialist.__init__(self, medical_aid_balance)
...
class Optometrist(Specialist):
    def __init__(self, medical_aid_balance, contact_lenses, spectacles):
        ...
        def medical_aid_claim(self, consultation, prescription, medical_tests, radiology_referral, blood_tests, medical_aid_balance, fee):
            Specialist.medical_aid_claim(self, consultation, prescription, medical_tests, radiology_referral, blood_tests, medical_aid_balance - fee)
            
        def medical_savings(self, amount, fee=0):
            self.fee = fee
            Specialist.medical_savings(self, amount+self.fee)
            
class GeneralSurgeon(Specialist):
    pass

patient0 = Dentist(10000, 400, 600)
patient0.proceedures(400, 600) ##Search how to call added functions of inherited classes

patient1 = Optometrist(7600, 50, 80)
patient1.medical_aid_claim(40, 70, 80, 90, 30)
patient1.medical_savings(500, fee=70) #"""BUGS"""
